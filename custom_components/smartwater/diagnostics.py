...
async def async_get_config_entry_diagnostics(hass: HomeAssistant, config_entry: ConfigEntry) -> dict[str, Any]:
    ...
    profile_id = config_entry.data[CONF_PROFILE_ID]
    profile_name = config_entry.data[CONF_PROFILE_NAME]
    _LOGGER.info(f"Retrieve diagnostics for install {profile_name} ({profile_id})")
    
    coordinator: SmartWaterCoordinator = SmartWaterCoordinatorFactory.create(hass, config_entry)
    diagnostics = {
        "config": {
...
DATACLASSES_EXCLUDE_NONE = ('SmartWaterHistoryItem', 'SmartWaterHistoryDetail')
...
def to_dict(obj: Any, dict_factory=dict) -> Any:
        ...
        try:
            if isinstance(obj, (int,float,str,NoneType)):
                ...
                df = dict_factory       # if not isinstance(obj, DATACLASSES_EXCLUDE_NONE) else SmartWaterDictFactory.exclude_none_values
